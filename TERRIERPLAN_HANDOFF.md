...
`courses`/`sections`/`bulletinPages` — public read, no client write. `users/{uid}` and its `plans`/`schedules` subcollections — owner-only read/write (`request.auth.uid == userId`). No changes here since the last handoff — notably, the new `offeringHistory` collection (added 2026-08-09–13) was never added to this file, so it has no rule at all (see Known Issues below).
...
8. **New (2026-08-14): `offeringHistory/{courseKey}` has no Firestore rule**, so it's unreadable from the client unlike every other reference-data collection. Not causing a visible bug today (nothing reads it yet) — see "Firestore data model" above.
